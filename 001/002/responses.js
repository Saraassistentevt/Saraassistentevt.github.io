...
"U": "<h2>Bem-vindo</h2 /",
"D": "<li>Bem-vindo</li>",
"Q": "<li><h1>Bem-vindo</h1></li>",

"F": "<span style='color: pink;'>Olá meu nome é Sara! como posso ajudá-lo?</span>",

      // vídeos
"Video": "<video width='320' height='240' controls autoplay><source src='img-vd-ad/pk.mp4' type='video/mp4'></video>",
...
"Imagem": "<img width='320' height='240' src='caminho_para_sua_imagem.jpg' alt='Descrição da imagem'>",
      ...
      // Áudios
"R": "<audio controls autoplay><source src='img-vd-ad/qwe.mp3' type='audio/mpeg'></audio>",
...
// puxar assunto em 28 minutos
var timeout;

function startConversation() {
    // Lista de possíveis assuntos aleatórios
    var randomTopics = [
    ...
    // Simular a resposta da assistente
...
// Função para iniciar o temporizador quando o usuário enviar uma mensagem
function onUserMessage() {
    resetTimeout();
    // Adicione aqui qualquer outra lógica que você precise executar quando o usuário enviar uma mensagem
}

// Chame a função resetTimeout() para iniciar o temporizador quando a página for carregada
resetTimeout();
...
// Função para reproduzir áudio de alerta após 5 minutos de inatividade
...
// Chame a função alertaInatividade() quando o chatbot estiver pronto para começar a monitorar a inatividade do usuário
alertaInatividade();

  
  
var idleTimer; // Variável para armazenar o temporizador

// Função para iniciar o temporizador de inatividade
function startIdleTimer() {
    idleTimer = setTimeout(function() {
        // Chamar a função de fazer pergunta após 30 minutos de inatividade
        askQuestion();
...
function resetIdleTimer() {
    clearTimeout(idleTimer); // Limpar o temporizador anterior
    startIdleTimer(); // Iniciar um novo temporizador
}

// Função para fazer uma pergunta ao usuário após 30 minutos de inatividade
function askQuestion() {
    // Aqui você pode definir a pergunta que deseja fazer ao usuário
    var question = "Olá! Parece que você se distraiu. Como posso ajudar?";
    simulateTyping(question); // Simular a digitação da pergunta
}

// Adicione um evento de escuta para reiniciar o temporizador quando houver interação do usuário
document.addEventListener("click", resetIdleTimer); // Você pode adicionar outros eventos aqui, como "keydown", "mousemove", etc.

// Iniciar o temporizador quando o script é carregado
startIdleTimer();
...
function checkResponse(userInput) {
    ...
    for (var i = 0; i < keys.length; i++) {
        if (userInput.toLowerCase() === keys[i].toLowerCase()) {
            return responses[keys[i]];
        }
    }
    return null; // Retorna null se não houver correspondência
}
function findBestMatchResponse(userInput) {
    var keys = Object.keys(responses);
    var bestMatch = { similarity: 0, response: "" };

    keys.forEach(function(key) {
        var similarity = calculateSimilarity(userInput, key);
        if (similarity > bestMatch.similarity) {
            bestMatch.similarity = similarity;
            bestMatch.response = responses[key];
        }
    });
...
function calculateSimilarity(str1, str2) {
    ...
    // Divide cada string em palavras
    var words1 = lowerStr1.split(" ");
    var words2 = lowerStr2.split(" ");

    // Conta o número de palavras comuns
    var commonWords = words1.filter(word => words2.includes(word));

    // Calcula a similaridade com base no número de palavras comuns
    var similarity = commonWords.length / Math.max(words1.length, words2.length);

    return similarity;
}


// função para saudar o usuário 1
...
// Chamar esta função ao iniciar o chat para exibir a saudação adequada
...
// Chame a função displayGreeting() ao iniciar o chat para exibir a saudação adequada
displayGreeting();
// função para saudar o usuário 2

function hideWords() {
    var allTextElements = document.querySelectorAll("body *"); // Seleciona todos os elementos dentro do body
    allTextElements.forEach(function(element) {
        element.classList.add("#"); // Adiciona a classe CSS "invisible-word" a cada elemento de texto
    });
}

// Chame a função hideWords() para tornar todas as palavras invisíveis
...
function simulateTyping(response) {
    ...
    botMessageElement.innerHTML = "Sara esta Digitando...";

    setTimeout(function() {
        botMessageElement.innerHTML = response; // Exibe a resposta após 1 minuto
...
function getBotResponse(userInput) {
    ...
    if (response) { 
        ...
        if (match) {
            ...
            var result;
            if (mathResponses[operator]) {
                result = mathResponses[operator](num1, num2);
                return "O resultado é " + result + ".";
            } else {
                return "Operador matemático inválido.";
            }
        } else {
            var firstLetter = userInput.charAt(0).toUpperCase();
            var keys = Object.keys(responses);
            for (var i = 0; i < keys.length; i++) {
                if (keys[i].charAt(0).toUpperCase() === firstLetter) {
                    return responses[keys[i]];
                }
            }
var audioNotFound = new Audio('img-vd-ad/Sara1.mp3');
        audioNotFound.play();
        return "Desculpe, não entendi sua pergunta, Ainda não estou completa, se foi uma pergunta sinples manda uma mensagem para o desenvolvedor Adicionar uma resposta!.. <a href='mailto:rfpds4880@gmail.com?subject=Olá gostaria de obter ajuda com?&body='>Clique aqui</a> para enviar sua pergunta por e-mail.";
          }
