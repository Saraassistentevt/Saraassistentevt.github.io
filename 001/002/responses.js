...
function loadVoices() {
    ...
    selectedVoice = voices.find(voice => voice.name.includes("female") || voice.name.includes("Feminina") || voice.name.includes("Google português do Brasil"));
        ...
        if (match) {
            ...
            for (var i = 0; i < keys.length; i++) {
                if (keys[i].charAt(0).toUpperCase() === firstLetter) {
                    simulateTyping(responses[keys[i]]);
                    return;
                }
            }
